...
class ConvModule(nn.Module):
    ...
    def __init__(self,
        ...
        if self.with_explicit_padding:
            ...
            self.padding_layer = BRICKS.build(pad_cfg, padding=padding)

        # reset padding to 0 for conv module
        conv_padding = 0 if self.with_explicit_padding else padding
        # build convolution layer
        self.conv = BRICKS.build(
        ...
        if self.with_norm:
            ...
            if norm_cfg.get('type', 'BN') == 'GN':
                self.gn = BRICKS.build(norm_cfg, num_channels=norm_channels)
                self.norm = 'gn'
            else:
                self.bn = BRICKS.build(norm_cfg, in_channels=norm_channels)
                self.norm = 'bn'
        else:
            self.norm = 'None'
        ...
        if self.with_activation:
            ...
            if act_cfg_['type'] not in [
                ...
                self.activate = BRICKS.build(act_cfg_)
