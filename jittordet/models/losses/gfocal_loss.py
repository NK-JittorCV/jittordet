...
def quality_focal_loss(pred, target, beta=2.0):
    ...
    zerolabel = jt.zeros(pred.shape, dtype=scale_factor.dtype)
...
def quality_focal_loss_tensor_target(pred, target, beta=2.0, activated=False):
    ...
    if activated:
        pred_sigmoid = pred
        loss_function = nn.binary_cross_entropy
    else:
        pred_sigmoid = pred.sigmoid()
...
def quality_focal_loss_with_prob(pred, target, beta=2.0):
    ...
    # positives are supervised by bbox quality (IoU) score
    scale_factor = score[pos] - pred_sigmoid[pos, pos_label]
    loss[pos, pos_label] = nn.binary_cross_entropy(
        pred[pos, pos_label], score[pos],
        reduction='none') * scale_factor.abs().pow(beta)
...
def distribution_focal_loss(pred, label):
    ...
    loss = nn.cross_entropy_loss(pred, dis_left, reduction='none') * w_left \
        + nn.cross_entropy_loss(pred, dis_right, reduction='none') * w_right
    return loss
