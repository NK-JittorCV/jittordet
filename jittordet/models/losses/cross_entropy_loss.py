# Modified from OpenMMLab. mmdet/models/losses/cross_entropy_loss.py
...
def _expand_onehot_labels(labels, label_weights, label_channels, ignore_index):
    ...
    bin_labels = jt.full((labels.size(0), label_channels),
                         0,
                         dtype=labels.dtype)
    valid_mask = (labels >= 0) & (labels != ignore_index)
    inds = jt.nonzero(valid_mask & (labels < label_channels))
...
def mask_cross_entropy(pred,
    ...
    return nn.binary_cross_entropy_with_logits(
        pred_slice, target, weight=class_weight, size_average=True)[None]
