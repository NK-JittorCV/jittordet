...
def deformable_im2col(data_im, data_offset, channels, height, width, ksize_h,
                      ksize_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
                      dilation_w, parallel_imgs, deformable_group,
                      columns_shape):
    ...
    return jt.code(
        columns_shape,
        data_im.dtype,
        inputs=[data_im, data_offset],
        cuda_header=HEADER,
        cuda_src=src)


def deformable_col2im_coord(data_col, data_im, data_offset, channels, height,
                            width, ksize_h, ksize_w, pad_h, pad_w, stride_h,
                            stride_w, dilation_h, dilation_w, parallel_imgs,
                            deformable_group, grad_offset_shape):
    src = f"""const int channels = {channels};
    const int height = {height};
    const int width = {width};
    const int ksize_h = {ksize_h};
    ...
    return jt.code(
        grad_offset_shape,
        data_offset.dtype, [data_col, data_im, data_offset],
        cuda_header=HEADER,
        cuda_src=src)


def deformable_col2im(data_col, data_offset, channels, height, width, ksize_h,
                      ksize_w, pad_h, pad_w, stride_h, stride_w, dilation_h,
                      dilation_w, parallel_imgs, deformable_group,
                      grad_im_shape):
    src = f"""const int channels = {channels};
    ...
    return jt.code(
        grad_im_shape,
        data_col.dtype, [data_col, data_offset],
        cuda_header=HEADER,
        cuda_src=src)


def deform_conv_forward_cuda(input, weight, offset, kW, kH, dW, dH, padW, padH,
    ...
    outputWidth = (inputWidth + 2 * padW - (dilationW *
                                            (kW - 1) + 1)) // dW + 1
    outputHeight = (inputHeight + 2 * padH - (dilationH *
                                              (kH - 1) + 1)) // dH + 1

    assert (offset.size(0) == batchSize), 'invalid batch size of offset'

    columns_shape = (nInputPlane * kW * kH,
                     im2col_step * outputHeight * outputWidth)

    input = input.view((batchSize // im2col_step, im2col_step, nInputPlane,
                        inputHeight, inputWidth))
    offset = offset.view(
        (batchSize // im2col_step, im2col_step, deformable_group * 2 * kH * kW,
    ...
    for elt in range(batchSize // im2col_step):
        columns = deformable_im2col(input[elt], offset[elt], nInputPlane,
                                    inputHeight, inputWidth, kH, kW, padH,
                                    padW, dH, dW, dilationH, dilationW,
        ...
        for g in range(group):
            output_buffer[elt, g] = (
                output_buffer[elt, g].flatten(1) +
                jt.matmul(weight[g].flatten(1), columns[g])).view_as(
                    output_buffer[elt, g])
    ...
    output_buffer = output_buffer.view(
        (batchSize // im2col_step, nOutputPlane, im2col_step, outputHeight,
         outputWidth))
...
def deform_conv_backward_input_cuda(input, offset, gradOutput, weight, kW, kH,
    ...
    for elt in range(batchSize // im2col_step):
        ...
        for g in range(group):
            columns[g] = jt.matmul(weight[g].flatten(1).transpose(1, 0),
        ...
        gradOffset[elt] = deformable_col2im_coord(
            columns, input[elt], offset[elt], nInputPlane, inputHeight,
            inputWidth, kH, kW, padH, padW, dH, dW, dilationH, dilationW,
            im2col_step, deformable_group, gradOffset[elt].shape)

        gradInput[elt] = deformable_col2im(columns, offset[elt], nInputPlane,
                                           inputHeight, inputWidth, kH, kW,
                                           padH, padW, dH, dW, dilationH,
                                           dilationW, im2col_step,
                                           deformable_group,
                                           gradInput[elt].shape)
...
def deform_conv_backward_parameters_cuda(input, offset, gradOutput, gradWeight,
                                         kW, kH, dW, dH, padW, padH, dilationW,
    ...
    for elt in range(batchSize // im2col_step):
        ...
        gradOutputBuffer = gradOutputBuffer.view(
            gradOutputBuffer.size(0), group,
            gradOutputBuffer.size(1) // group, gradOutputBuffer.size(2),
            gradOutputBuffer.size(3))
        columns = columns.view(group,
                               columns.size(0) // group, columns.size(1))
        gradWeight = gradWeight.view(group,
                                     gradWeight.size(0) // group,
                                     gradWeight.size(1), gradWeight.size(2),
                                     gradWeight.size(3))

        for g in range(group):
            gradWeight[g] = (
                gradWeight[g].flatten(1) +
                scale * jt.matmul(gradOutputBuffer[elt, g].flatten(1),
...
class DeformConvFunction(jt.Function):
    ...
    def execute(self,
        ...
        output_shape = DeformConvFunction._output_size(input, weight,
                                                       self.padding,
                                                       self.dilation,
                                                       self.stride)
        ...
        if not jt.flags.use_cuda:
            ...
            output = deform_conv_forward_cuda(
                input, weight, offset, weight.size(3), weight.size(2),
                self.stride[1], self.stride[0], self.padding[1],
                self.padding[0], self.dilation[1], self.dilation[0],
                self.groups, self.deformable_groups, cur_im2col_step)
    ...
    def grad(self, grad_output):
        ...
        if not jt.flags.use_cuda:
            ...
            grad_weight = deform_conv_backward_parameters_cuda(
                input, offset, grad_output, grad_weight, weight.size(3),
                weight.size(2), self.stride[1], self.stride[0],
                self.padding[1], self.padding[0], self.dilation[1],
