...
def multiclass_poly_nms(bboxes, scores, labels, thresh):
    ...
    keep = poly_nms(
        jt.concat([bboxes_for_nms, scores[:, None]], dim=1), thresh)
...
def test2(i):
    ...
    bboxes, scores, labels = pickle.load(
        open(f'/home/lxl/diff/poly_nms_in_{i}.pkl', 'rb'))
    bboxes_j, scores_j, keep_j = multiclass_poly_nms(
        jt.array(bboxes), jt.array(scores), jt.array(labels), 0.1)

    bboxes_t, scores_t, keep_t = pickle.load(
        open(f'/home/lxl/diff/poly_nms_out_{i}.pkl', 'rb'))
    # print(keep_t)
    # print(keep_j)
    ...
    if keep_j.numpy().shape != keep_t.shape or not np.allclose(
            keep_j.numpy(), keep_t, rtol=1e-05):
